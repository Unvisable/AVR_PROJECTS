...
static void (*EXTICopy_pvCallBack[3])(void)={0}; /*pointer to array function for INT0 & INT1 & INT2*/
...
uint8 EXTI_u8SetCallBack(IntChannel_t Copy_IntCh, void (*Copy_pvCallBackFunc)(void))
{
	...
	{
       EXTICopy_pvCallBack[Copy_IntCh] = Copy_pvCallBackFunc;
	}
	else
	{
...
{
  if(EXTICopy_pvCallBack[INT0] !=NULL)
  {
	  EXTICopy_pvCallBack[INT0]();
  }
...
{
	if(EXTICopy_pvCallBack[INT1] !=NULL)
	  {
		  EXTICopy_pvCallBack[INT1]();
	  }
}
...
{
	if(EXTICopy_pvCallBack[INT2] !=NULL)
	  {
		  EXTICopy_pvCallBack[INT2]();
	  }
}
